...
export default function HistoryView() {
  ...
  const calibration = useMemo(() => {
    const arr = items.map((it) => {
      // placeholders for future outcome computation
      // crude rule: TP before Stop with forecast direction based on verdict
      const direction = it.ai.verdict === 'Sell' ? -1 : it.ai.verdict === 'Buy' ? 1 : 0;
      const outcome: 0 | 1 = direction === 0 ? 0 : ((): 0 | 1 => {
        // we don't have intra-minute ticks here; approximate with closes
        return 0; // unknown -> conservative 0
      })();
