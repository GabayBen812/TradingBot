...
export function estimateSlippage(levels: DepthLevel[], notional: number, side: 'buy' | 'sell'): { avgPrice: number; filledNotional: number } {
  const book = side === 'buy' ? levels.slice(0) : levels.slice(0);
  let remaining = notional;
  let cost = 0;
  for (const lvl of book) {
    const canFill = lvl.qty * lvl.price;
    const take = Math.min(remaining, canFill);
    cost += take; // in notional, price-weighted implicitly
    remaining -= take;
