...
export async function runEnsemble(params: { symbol: string; interval: string; multiTF: MultiTF; notes?: string; plan?: UserPlan }): Promise<AIResp> {
  const prompts = buildPromptVariants({ symbol: params.symbol, interval: params.interval, taMultiTF: params.multiTF, userNote: params.notes, userPlan: params.plan });
  ...
  if (valids.length === 0) {
    const errors = [a, b, c]
      ...
      .map((r) => (r.reason?.message ?? String(r.reason)) as string);
    throw new Error('All ensemble calls failed: ' + errors.join(' | '));
  }
